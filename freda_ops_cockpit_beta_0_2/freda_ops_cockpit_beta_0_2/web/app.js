(function () {
  ...
  async function syncReporting() {
    ...
    try {
      ...
      state.live = body.live || state.live;
      state.serverOnline = true;
      alert('Reporting.site sync complete.');
    } catch (error) {
      alert('Reporting.site sync could not run yet. Configure REPORTING_PHPSESSID in server/.env or deployment secrets.\n\n' + error.message);
      state.lastError = error.message;
    } finally {
